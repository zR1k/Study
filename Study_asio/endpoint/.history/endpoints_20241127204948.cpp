...
int send_data_by_send(){
    ...
    try{
        ...
        sock.connect(ep);
        std::string buf = "hello world";
        int send_length = sock.send(boost::asio::buffer(buf), buf.length());
        if(send <= 0){
            return 0;
        }
    }catch(boost::system::system_error &e){
...
int read_data_by_receive(){
    ...
    try{
        ...
        const unsigned char MESSAGE_SIZE = 7;
        char buf[MESSAGE_SIZE];
        int read_length = sock.receive(boost::asio::buffer(buf), MESSAGE_SIZE);
...
int read_data_by_read(){
    ...
    try{
        ...
        const unsigned char MESSAGE_SIZE = 7;
        char buf[MESSAGE_SIZE];
        int read_length = boost::asio::read(sock, boost::asio::buffer(buf, MESSAGE_SIZE));
